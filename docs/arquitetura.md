...
- `accounts`: dominio de contas financeiras. Possui o model `Account`, vinculado ao usuario e com tipo, saldo e status ativo.
...
Os apps `categories` e `transactions` ainda existem como scaffold Django. O app `accounts` possui apenas o model de dominio; ainda nao possui admin customizado, views, URLs, templates, migrations ou regras de CRUD implementadas.
