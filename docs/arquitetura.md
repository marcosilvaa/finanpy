...
- `accounts`: dominio de contas financeiras. Possui o model `Account`, vinculado ao usuario e com tipo, saldo e status ativo. Tambem possui admin customizado para gerenciamento via Django Admin.
...
Os apps `categories` e `transactions` ainda existem como scaffold Django. O app `accounts` possui model de dominio e admin customizado; ainda nao possui views, URLs, templates, migrations ou regras de CRUD implementadas.
